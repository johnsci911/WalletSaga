<div class="flex flex-col items-center pb-4 px-4 mx-auto mt-20 max-w-7xl lg:px-0">
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($goals as $goal)
                    <div
                        class="bg-slate-700 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-shadow {{ $goal['is_completed'] ? 'opacity-75' : '' }} flex flex-col"
                    >
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex-1">
                                    <h3
                                        class="text-lg font-bold text-slate-100 {{ $goal['is_completed'] ? 'line-through' : '' }}"
                                    >
                                        {{ $goal['name'] }}
                                    </h3>
                                    @if ($goal['category'])
                                        <span
                                            class="inline-block mt-1 px-2 py-1 text-xs bg-slate-600 text-slate-300 rounded-full"
                                        >
                                            {{ $goal['category'] }}
                                        </span>
                                    @endif
                            ...
                            @if ($goal['deadline'])
                                <div
                                    class="flex items-center text-sm mb-4 {{ $goal['is_overdue'] ? 'text-red-400' : 'text-slate-400' }}"
                                >
                                    ...
                                    <span>
                                        ...
                                        @else
                                            {{ round($goal['days_remaining']) }} days remaining
                                            ({{ $goal['deadline'] }})
                                        @endif
                                    </span>
                                </div>
                ...
                <nav role="navigation" aria-label="Pagination Navigation">
                    ...
                    <div class="flex flex-col items-center gap-3 sm:hidden">
                        <span class="text-sm text-slate-400">
                            Showing {{ $goals->firstItem() }} to {{ $goals->lastItem() }} of {{ $goals->total() }}
                            results
                        </span>
                        <div class="flex gap-2">
                    ...
                    <div class="hidden sm:flex items-center justify-between">
                        ...
                        <div class="flex gap-1">
                            ...
                            @if ($goals->onFirstPage())
                                <span
                                    class="px-3 py-2 text-sm font-medium bg-slate-700 text-slate-500 border border-slate-600 cursor-default rounded-md"
                                >
                                    &laquo;
                                </span>
                            @else
                                <button
                                    wire:click="previousPage"
                                    class="px-3 py-2 text-sm font-medium bg-slate-700 text-slate-300 border border-slate-600 hover:bg-slate-600 rounded-md transition-colors"
                                >
                                    &laquo;
                                </button>
                            @endif
    ...
    <!-- Goal Modal -->
    <x-goal-modal :goalForm="$goalForm" :editingGoalId="$editingGoalId" />
</div>
